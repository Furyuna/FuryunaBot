...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (!targetMember.roles.cache.has(localConfig.roles.verifiedMember)) {
            ...
            return interaction.reply({ content: localConfig.messages.zatenDogrulanmamis(targetMember.id, roleName), ephemeral: true });
        }

        await targetMember.roles.remove(localConfig.roles.verifiedMember);
        await targetMember.roles.add(localConfig.roles.newMember);
        await interaction.reply({ content: localConfig.messages.dogrulamaSilindi(targetMember.id, interaction.user.id) });
    }
};